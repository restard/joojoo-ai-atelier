...
```text
次のdeck JSONを文字列として `deck_json` に入れて、generateDeckを呼んでください。
...
- GPT Previewから `generateDeck` が呼べる
- API側ログに `/generate` のPOSTが出る
- `dist/api/` にPPTXが保存される
- ChatGPT側でPPTXレスポンスが扱えるか確認できる
...
その後もCustom GPT Actions側で `generateDeck` のPOSTがAPIへ届かなかったため、GET版の `generateDeckFromJsonQuery` も追加した。6/8時点の接続確認では、POSTの `generateDeck` より先にGETの `generateDeckFromJsonQuery` を使う。
