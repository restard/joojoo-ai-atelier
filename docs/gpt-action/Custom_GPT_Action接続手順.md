# Custom GPT Action接続手順

最終更新: 2026年5月29日

## 目的

Custom GPTからgenDeck用のdeck JSONを作り、Render APIまたはフォールバックのローカルAPIでPPTX生成までつなぐ。
...
- Actionsで使う本線はRender API: `https://gen-deck.onrender.com/openapi.json`
- Actionsは `generateSmokeDeck` と `generateDeckFromJsonQuery` の2つだけにする
- 通常チャットで `ClientResponseError` が表示されても、Render logsで `/generate-json` が200ならAPI生成自体は成功として扱う
- 6/8の成功条件は、ChatGPT内添付ではなく `download_url` 取得まで

## 事前に起動するもの（ngrokフォールバック時）
